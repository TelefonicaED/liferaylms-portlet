...
document.getElementsBySelector = function(cssSelector, root) {
	var all = root ? root.getElementsByTagName('*') : document
	...
	if (window.navigator.userAgent.indexOf('MSIE') >= 0) {
		...
		if (!matchValue) {
			matchValue = (window.navigator.userAgent.indexOf('Opera') >= 0) ? '"ssCssMatch"'
					: 'ssCssMatch 1';
			document.getElementsBySelector.matchValue = matchValue;
		}
		styleSheet.insertRule(cssSelector
				+ ' { counter-increment: ssCssMatch }', 0);
...
Number.parse = function(s) {
	...
	if ((s.indexOf('.') >= 0) || (s.indexOf('e') >= 0) || s.endsWith('f')
			|| s.endsWith('F')) {
		return parseFloat(s);
	}
	return parseInt(s);
...
Number.prototype._netFormat = function(format, useLocale) {
	var nf = useLocale ? CultureInfo.Current.numberFormat
			: CultureInfo.Neutral.numberFormat;
	var s = '';
	var precision = -1;
	if (format.length > 1) {
	...
	case 'N':
		...
		if (precision && (nf.numberDecimalSeparator != '.')) {
			var index = s.indexOf('.');
			s = s.substr(0, index) + nf.numberDecimalSeparator
					+ s.substr(index + 1);
		}
		if ((fs == 'n') || (fs == 'N')) {
			s = Number._commaFormat(s, nf.numberGroupSizes,
	...
	case 'C':
		...
		if (precision && (nf.currencyDecimalSeparator != '.')) {
			var index = s.indexOf('.');
			s = s.substr(0, index) + nf.currencyDecimalSeparator
					+ s.substr(index + 1);
		}
		s = Number._commaFormat(s, nf.currencyGroupSizes,
	...
	case 'P':
		...
		if (precision && (nf.percentDecimalSeparator != '.')) {
			var index = s.indexOf('.');
			s = s.substr(0, index) + nf.percentDecimalSeparator
					+ s.substr(index + 1);
		}
		s = Number._commaFormat(s, nf.percentGroupSizes,
...
Date.prototype._netFormat = function(format, useLocale) {
	var dtf = useLocale ? CultureInfo.Current.dateFormat
			: CultureInfo.Neutral.dateFormat;
	var useUTC = false;
	if (format.length == 1) {
		switch (format) {
		case 'f':
	...
	var dt = this;
	if (useUTC) {
		dt = new Date(Date.UTC(dt.getUTCFullYear(), dt.getUTCMonth(), dt
				.getUTCDate(), dt.getUTCHours(), dt.getUTCMinutes(), dt
				.getUTCSeconds(), dt.getUTCMilliseconds()));
	}
	re.lastIndex = 0;
	while (true) {
		...
		case 'zzz':
			part = dt.getTimezoneOffset() / 60;
			part = ((part >= 0) ? '-' : '+')
					+ Math.floor(Math.abs(part)).toString().padLeft(2, '0');
			if (fs == 'zzz') {
				part += dtf.timeSeparator
						+ Math.abs(dt.getTimezoneOffset() % 60).toString()
								.padLeft(2, '0');
			}
			break;
		}
	...
	default:
		...
		if (IArray.isInstance(object)) {
			...
			for ( var i = 0; i < length; i++) {
				Debug._dumpCore(sb, object.getItem(i), '[' + i + ']',
						recursiveIndentation, dumpedObjects);
			}
		} else {
			if (object.tagName) {
				sb
						.appendLine(indentation + name + ': <' + object.tagName
								+ '>');
				var attributes = object.attributes;
				for ( var i = 0; i < attributes.length; i++) {
					var attrValue = attributes[i].nodeValue;
					if (attrValue) {
						Debug._dumpCore(sb, attrValue, attributes[i].nodeName,
								recursiveIndentation, dumpedObjects);
					}
				}
			} else {
				...
				for ( var field in object) {
					...
					if (!Function.isInstance(v)) {
						Debug._dumpCore(sb, v, field, recursiveIndentation,
								dumpedObjects);
					}
				}
			}
...
Type.createNamespace = function(name) {
	...
	for ( var i = 0; i < nameParts.length; i++) {
		...
		var nso = ns[part];
		if (!nso) {
			ns[part] = nso = new __Namespace(nameParts.slice(0, i + 1)
					.join('.'));
			if (i == 0) {
...
ScriptLoader.prototype = {
	...
	dispose : function() {
		if (this._scriptElements) {
			for ( var i = 0; i < this._scriptElements.length; i++) {
				...
				if (this._isIE) {
					scriptElement.detachEvent('onreadystatechange',
							this._onLoadHandler);
				} else {
	...
	load : function(loadInParallel, timeout, loadedHandler, errorHandler) {
		...
		if (timeout) {
			window.setTimeout(Delegate.create(this, this._onScriptError),
					timeout);
		}
	},
	_loadScript : function(scriptURL) {
		...
		if (this._isIE) {
			scriptElement
					.attachEvent('onreadystatechange', this._onLoadHandler);
		} else {
...
ScriptHost.add_loaded = function(value) {
	if (ScriptHost._loaded) {
		...
		ScriptHost._loadedHandler = Delegate.combine(ScriptHost._loadedHandler,
				value);
	}
}
ScriptHost.remove_loaded = function(value) {
	ScriptHost._loadedHandler = Delegate.remove(ScriptHost._loadedHandler,
			value);
}
ScriptHost.add_loading = function(value) {
	ScriptHost._loadingHandler = Delegate.combine(ScriptHost._loadingHandler,
			value);
}
ScriptHost.remove_loading = function(value) {
	ScriptHost._loadingHandler = Delegate.remove(ScriptHost._loadingHandler,
			value);
}
ScriptHost.add_unload = function(value) {
	ScriptHost._unloadHandler = Delegate.combine(ScriptHost._unloadHandler,
			value);
}
ScriptHost.remove_unload = function(value) {
	ScriptHost._unloadHandler = Delegate.remove(ScriptHost._unloadHandler,
			value);
}
...
ScriptHost._loadScripts = function() {
	if (ScriptHost._coreScripts != null) {
		ScriptHost._coreScripts.load(false, 20 * 1000, Delegate.create(null,
				ScriptHost._onScriptsCompleted), Delegate.create(null,
				ScriptHost._onScriptsError));
		return;
	} else {
		ScriptHost._fxScripts.load(true, 20 * 1000, Delegate.create(null,
				ScriptHost._onScriptsCompleted), Delegate.create(null,
				ScriptHost._onScriptsError));
	}
}
...
ScriptHost._windowUnloadHandler = Delegate.create(null,
		ScriptHost._onWindowUnload);
