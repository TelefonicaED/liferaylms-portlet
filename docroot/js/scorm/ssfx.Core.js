...
ScriptFX.CollectionChangedAction.createEnum('ScriptFX.CollectionChangedAction',
		false);
...
ScriptFX.INotifyPropertyChanged
		.createInterface('ScriptFX.INotifyPropertyChanged');
...
ScriptFX.Application = function() {
	...
	if ($1.startsWith('$')) {
		var $2 = this.get_host();
		$1 = $1.replace('$browser', Enum.toString(ScriptFX.HostName, $2
				.get_name()));
...
ScriptFX.Application.prototype = {
	...
	getService : function(serviceType) {
		if ((serviceType === IServiceContainer)
				|| (serviceType === ScriptFX.IEventManager)) {
			return this;
		}
		if (this.$10 != null) {
...
ScriptFX.JSON.$1 = function($p0, $p1) {
	...
	case 'object':
		if (Array.isInstance($p1)) {
			...
			var $5 = $p1;
			var $6 = Date.UTC($5.getUTCFullYear(), $5.getUTCMonth(), $5
					.getUTCDate(), $5.getUTCHours(), $5.getUTCMinutes(), $5
					.getUTCSeconds(), $5.getUTCMilliseconds());
			$p0.append('\"\\@');
...
ScriptFX.ObservableCollection.prototype = {
	...
	add : function(item) {
		...
		this.$1.add(item);
		if (this.$3 != null) {
			this.$3.invoke(this, new ScriptFX.CollectionChangedEventArgs(0,
					item));
		}
	},
	clear : function() {
		if (this.$1.length !== 0) {
			...
			this.$1.clear();
			if (this.$3 != null) {
				this.$3.invoke(this, new ScriptFX.CollectionChangedEventArgs(2,
						null));
			}
		}
	},
	...
	remove : function(item) {
		if (this.$1.contains(item)) {
			...
			this.$1.remove(item);
			if (this.$3 != null) {
				this.$3.invoke(this, new ScriptFX.CollectionChangedEventArgs(1,
						item));
			}
		}
	}
...
ScriptFX.Net.HTTPRequestState
		.createEnum('ScriptFX.Net.HTTPRequestState', false);
...
ScriptFX.Net.HTTPRequestManager.add_requestInvoking = function(value) {
	ScriptFX.Net.HTTPRequestManager.$0 = Delegate.combine(
			ScriptFX.Net.HTTPRequestManager.$0, value);
}
ScriptFX.Net.HTTPRequestManager.remove_requestInvoking = function(value) {
	ScriptFX.Net.HTTPRequestManager.$0 = Delegate.remove(
			ScriptFX.Net.HTTPRequestManager.$0, value);
}
ScriptFX.Net.HTTPRequestManager.add_requestInvoked = function(value) {
	ScriptFX.Net.HTTPRequestManager.$1 = Delegate.combine(
			ScriptFX.Net.HTTPRequestManager.$1, value);
}
ScriptFX.Net.HTTPRequestManager.remove_requestInvoked = function(value) {
	ScriptFX.Net.HTTPRequestManager.$1 = Delegate.remove(
			ScriptFX.Net.HTTPRequestManager.$1, value);
}
...
ScriptFX.Net.HTTPRequestManager.$6 = function($p0) {
	...
	$1.invoke();
	if (((ScriptFX.Net.HTTPRequestManager.$2 !== 0) || ($p0.get_timeout() !== 0))
			&& (ScriptFX.Net.HTTPRequestManager.$4 == null)) {
		ScriptFX.Net.HTTPRequestManager.$4 = Delegate.create(null,
				ScriptFX.Net.HTTPRequestManager.$8);
		ScriptFX.Application.current
				.add_idle(ScriptFX.Net.HTTPRequestManager.$4);
	}
}
ScriptFX.Net.HTTPRequestManager.$7 = function($p0, $p1, $p2) {
	...
	if ((ScriptFX.Net.HTTPRequestManager.$3.length === 0)
			&& (ScriptFX.Net.HTTPRequestManager.$4 != null)) {
		ScriptFX.Application.current
				.remove_idle(ScriptFX.Net.HTTPRequestManager.$4);
		ScriptFX.Net.HTTPRequestManager.$4 = null;
	}
}
...
ScriptFX.Net.HTTPTransport.createURI = function(uri, transportType, parameters) {
	if (parameters == null) {
		return '{__nullParams: true, __uri:\'' + uri + '\', __transportType: '
...
ScriptFX.Net._Core$3.prototype = {
	...
	invoke : function() {
		...
		if (!this.get_request().get_hasCredentials()) {
			...
			this.$1.open(this.getMethod(), $0.get_URI(), true, $0
					.get_userName(), $0.get_password());
		}
		...
		if (($2 != null) && (($1 == null) || ($1['Content-Type'] == null))) {
			this.$1.setRequestHeader('Content-Type',
...
ScriptFX.UI.AnimationStopState.createEnum('ScriptFX.UI.AnimationStopState',
		false);
...
ScriptFX.UI.AnimationManager.$3 = function() {
	...
	if ($2.length !== 0) {
		...
		if (ScriptFX.UI.AnimationManager.$2 === 0) {
			ScriptFX.UI.AnimationManager.$2 = window.setTimeout(Delegate
					.create(null, ScriptFX.UI.AnimationManager.$3),
					1000 / ScriptFX.UI.AnimationManager.$0);
		}
	}
}
ScriptFX.UI.AnimationManager.$4 = function($p0) {
	...
	$p0.$D(false);
	if (ScriptFX.UI.AnimationManager.$2 === 0) {
		ScriptFX.UI.AnimationManager.$2 = window.setTimeout(Delegate.create(
				null, ScriptFX.UI.AnimationManager.$3),
				1000 / ScriptFX.UI.AnimationManager.$0);
	}
}
...
ScriptFX.UI.Behavior = function(domElement, id) {
	...
	if (!String.isNullOrEmpty(id)) {
		if (id === 'control') {
			var $1 = domElement[id];
			if (($1 != null)
					&& (Type.getInstanceType($1) === ScriptFX.UI._Core$4)) {
...
ScriptFX.UI.Behavior.prototype = {
	...
	raisePropertyChanged : function(propertyName) {
		...
		if ($0 != null) {
			$0
					.invoke(this, new ScriptFX.PropertyChangedEventArgs(
							propertyName));
		}
	}
}
...
ScriptFX.UI.Color.parse = function(s) {
	...
	if ((s.length === 7) && s.startsWith('#')) {
		...
		if ($3.length === 3) {
			return new ScriptFX.UI.Color(parseInt($3[0].trim()), parseInt($3[1]
					.trim()), parseInt($3[2].trim()));
		}
	}
	return null;
}
...
ScriptFX.UI.DragDropManager.add_dragDropEnding = function(value) {
	ScriptFX.UI.DragDropManager.$3 = Delegate.combine(
			ScriptFX.UI.DragDropManager.$3, value);
}
ScriptFX.UI.DragDropManager.remove_dragDropEnding = function(value) {
	ScriptFX.UI.DragDropManager.$3 = Delegate.remove(
			ScriptFX.UI.DragDropManager.$3, value);
}
ScriptFX.UI.DragDropManager.add_dragDropStarting = function(value) {
	ScriptFX.UI.DragDropManager.$2 = Delegate.combine(
			ScriptFX.UI.DragDropManager.$2, value);
}
ScriptFX.UI.DragDropManager.remove_dragDropStarting = function(value) {
	ScriptFX.UI.DragDropManager.$2 = Delegate.remove(
			ScriptFX.UI.DragDropManager.$2, value);
}
ScriptFX.UI.DragDropManager.$5 = function() {
	if (ScriptFX.UI.DragDropManager.$3 != null) {
		ScriptFX.UI.DragDropManager.$3.invoke(null,
				new ScriptFX.UI.DragDropEventArgs(
						ScriptFX.UI.DragDropManager.$4));
	}
...
ScriptFX.UI.DragDropManager.startDragDrop = function(data, dragVisual,
		dragOffset, source, context) {
	...
	if (ScriptFX.UI.DragDropManager.$2 != null) {
		ScriptFX.UI.DragDropManager.$2.invoke(null,
				new ScriptFX.UI.DragDropEventArgs(data));
	}
	ScriptFX.UI.DragDropManager.$0
			.dragDrop(new ScriptFX.UI._Core$0(source), context, $0, dragVisual,
					dragOffset, ScriptFX.UI.DragDropManager.$4);
	return true;
}
...
ScriptFX.UI.Element.getBounds = function(element) {
	...
	return ScriptFX.UI.$create_Bounds($0.left, $0.top, element.offsetWidth,
...
ScriptFX.UI.OverlayBehavior = function(domElement, options) {
	...
	$0.width = '100%';
	if (ScriptFX.Application.current.get_isIE()
			&& (ScriptFX.Application.current.get_host().get_majorVersion() < 7)) {
		$0.position = 'absolute';
	} else {
		this.$8 = true;
		$0.position = 'fixed';
	...
	if (options.fadeInOutInterval !== 0) {
		this.$9 = new ScriptFX.UI.FadeEffect(this.$7,
				options.fadeInOutInterval, options.opacity);
		this.$9.set_easingFunction(Delegate.create(null,
				ScriptFX.UI.TimedAnimation.easeInOut));
...
ScriptFX.UI.OverlayBehavior.prototype = {
	...
	hide : function() {
		...
		if (this.$9 != null) {
			...
			this.$B = false;
			var $0 = this.get_events().getHandler(
					ScriptFX.UI.OverlayBehavior.$6);
			if ($0 != null) {
				$0.invoke(this, EventArgs.Empty);
			}
		}
...
ScriptFX.UI.PopupBehavior.prototype = {
	...
	show : function() {
		...
		ScriptFX.UI.Element.setLocation(this.get_domElement(), ScriptFX.UI
				.$create_Location($1, $2));
		...
		if (($C.get_name() === 1) && ($C.get_majorVersion() < 7)) {
			...
			this.get_domElement().parentNode.insertBefore(this.$7, this
					.get_domElement());
		}
	}
}
...
ScriptFX.Application.createClass('ScriptFX.Application', null,
		IServiceProvider, IServiceContainer, ScriptFX.IEventManager);
ScriptFX.CancelEventArgs.createClass('ScriptFX.CancelEventArgs', EventArgs);
ScriptFX.CollectionChangedEventArgs.createClass(
		'ScriptFX.CollectionChangedEventArgs', EventArgs);
ScriptFX.ApplicationUnloadingEventArgs.createClass(
		'ScriptFX.ApplicationUnloadingEventArgs', EventArgs);
ScriptFX.HistoryManager.createClass('ScriptFX.HistoryManager', null,
		IDisposable);
...
ScriptFX.PropertyChangedEventArgs.createClass(
		'ScriptFX.PropertyChangedEventArgs', EventArgs);
ScriptFX.ObservableCollection.createClass('ScriptFX.ObservableCollection',
		null, IDisposable, IArray, IEnumerable,
		ScriptFX.INotifyCollectionChanged);
ScriptFX.Net.HTTPRequest.createClass('ScriptFX.Net.HTTPRequest', null,
		IDisposable);
ScriptFX.Net.HTTPRequestManager.createClass('ScriptFX.Net.HTTPRequestManager');
ScriptFX.Net.HTTPTransport.createClass('ScriptFX.Net.HTTPTransport', null,
		IDisposable);
ScriptFX.Net.PostHTTPRequestEventArgs.createClass(
		'ScriptFX.Net.PostHTTPRequestEventArgs', EventArgs);
ScriptFX.Net.PreHTTPRequestEventArgs.createClass(
		'ScriptFX.Net.PreHTTPRequestEventArgs', EventArgs);
ScriptFX.Net._Core$2.createClass('ScriptFX.Net._Core$2', null,
		ScriptFX.Net.IHTTPResponse);
ScriptFX.Net._Core$3.createClass('ScriptFX.Net._Core$3',
...
ScriptFX.UI.AnimationSequence.createClass('ScriptFX.UI.AnimationSequence',
		ScriptFX.UI.Animation);
ScriptFX.UI.Behavior.createClass('ScriptFX.UI.Behavior', null, IDisposable,
		ScriptFX.ISupportInitialize, ScriptFX.INotifyPropertyChanged);
ScriptFX.UI.Color.createClass('ScriptFX.UI.Color');
ScriptFX.UI.Control.createClass('ScriptFX.UI.Control', ScriptFX.UI.Behavior,
		ScriptFX.INotifyDisposing);
ScriptFX.UI.DOMEventList.createClass('ScriptFX.UI.DOMEventList', null,
		IDisposable);
ScriptFX.UI.DragDropEventArgs.createClass('ScriptFX.UI.DragDropEventArgs',
		EventArgs);
ScriptFX.UI.DragDropManager.createClass('ScriptFX.UI.DragDropManager');
ScriptFX.UI._Core$0.createClass('ScriptFX.UI._Core$0', null,
		ScriptFX.UI.IDragSource);
ScriptFX.UI.Element.createClass('ScriptFX.UI.Element');
ScriptFX.UI.TimedAnimation.createClass('ScriptFX.UI.TimedAnimation',
		ScriptFX.UI.Animation);
ScriptFX.UI.FadeEffect.createClass('ScriptFX.UI.FadeEffect',
		ScriptFX.UI.TimedAnimation);
ScriptFX.UI._Core$4.createClass('ScriptFX.UI._Core$4', ScriptFX.UI.Control);
ScriptFX.UI.OverlayBehavior.createClass('ScriptFX.UI.OverlayBehavior',
		ScriptFX.UI.Behavior);
ScriptFX.UI.PopupBehavior.createClass('ScriptFX.UI.PopupBehavior',
		ScriptFX.UI.Behavior);
