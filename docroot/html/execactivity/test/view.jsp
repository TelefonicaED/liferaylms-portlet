...
if((Validator.isNotNull(navigateParam))||(StringPool.BLANK.equals(password))||(passwordParam.equals(password))){
	...
	<%
		...
		<aui:form name="formulario" action="<%=!hasPermissionAccessCourseFinished ? correctURL : correctAccessFinishedURL %>" method="post" role="form" onSubmit="javascript:return false;">
			...
			if (questions.size() > 0){
				...
				<aui:input type="hidden" name="improve" value='<%= new Boolean(ParamUtil.getBoolean(renderRequest, "improve",false)).toString() %>' />
			<%}  else {%>
	...
	if(ParamUtil.getBoolean(renderRequest, "improve",false)) {
